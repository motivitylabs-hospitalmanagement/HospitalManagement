...
<h3></h3><tr bgcolor="wheat"><td>doctor name</td> <td>special </td><td>experience</td><td>apply appointment</td></tr></h3>
	<%
		...
		{
	%>
	<tr><td><%=dr.getFirstname()+" "+dr.getLastname() %></td> <td><%=dr.getDepartment() %></td><td><%=dr.getExp()%></td>
	<td><button><a href ="apply_doctor_appointment.jsp?dname=<%=dr.getFirstname()%>&dgender=<%=dr.getGender()%>&dphno=<%=dr.getPhoneno()%>">apply appointment</a></button></td>
